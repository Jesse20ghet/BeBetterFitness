...
public class LiftCollection
{
    ...
    public void LoadAll()
    {
        ...
        {
            Lift liftToAdd = new Lift(_context);
    ...
    public void LoadLiftsAssociatedToMuscleId(int muscleId)
    {
        _collection.clear();

        String[] selectionArgs = new String[1];
        selectionArgs[0] = Integer.toString(muscleId);
        Cursor cursor = db.getReadableDatabase().rawQuery("SELECT Lift._id, LiftName, Description FROM Lift JOIN LiftMuscle ON Lift._id = LiftMuscle.liftId WHERE muscleId = ?", selectionArgs);
    ...
    public Lift FindLiftByLiftName(String liftName)
    {
        ...
        for(int i = 0; i < _collection.size(); i++)
        {
            if(_collection.get(i).LiftName.equalsIgnoreCase(liftName))
            {
                liftToReturn = _collection.get(i);
                break;
            }
    ...
    public Lift FindLiftByLiftNameAndId(String liftName, int currentLiftId)
    {
        Lift liftToReturn = new Lift(_context);
        liftToReturn.Id = -1;
        for(int i = 0; i < _collection.size(); i++)
        {
            if(_collection.get(i).LiftName.equalsIgnoreCase(liftName) && _collection.get(i).Id != currentLiftId)
            {
                liftToReturn = _collection.get(i);
                break;
            }
